...
package org.tensorflow.ext.tracking;
...
import org.tensorflow.ext.classifier.Classifier.Recognition;
...
public class MultiBoxTracker {
    ...
    public synchronized void draw(final Canvas canvas) {
        ...
        if (sensorOrientation == Surface.ROTATION_90) {//landscape
            ...
            multiplier = Math.min(canvas.getWidth() / (float) frameHeight, canvas.getHeight() / (float) frameWidth);
            frameToCanvasMatrix =
                    ImageUtils.getTransformationMatrix(
                            frameWidth,
                            frameHeight,
    ...
    private void handleDetection(final byte[] frameCopy, final long timestamp, final Pair<Float, Recognition> potential) {
        final ObjectTracker.TrackedObject potentialObject = objectTracker.trackObject(potential.second.getLocation(), timestamp, frameCopy);
